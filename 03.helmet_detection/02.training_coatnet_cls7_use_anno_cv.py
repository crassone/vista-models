...
result_dir = f'{root_dir}/data/outputs/03.helmet_detection/02.training_coatnet_cls7_use_anno'
    ...
    for weight, class_names in zip(weight_class, class_category):
        for _ in range(weight):
            train_weight_list.append(
                pd.concat([train_df[train_df['label'] == class_name] for class_name in class_names])
            )
...
def train_cv(annotation_head_df: pd.DataFrame, pred_head_df: pd.DataFrame):
    annotation_head_df = copy.deepcopy(annotation_head_df[annotation_head_df['label'].notnull()])
    for num_cv in range(3):
        train_df = copy.deepcopy(pd.concat([
            annotation_head_df[
                (annotation_head_df['validation'] != num_cv) &
                (annotation_head_df['validation'] != 999)
            ],
            pred_head_df[
                (pred_head_df['validation'] != num_cv) &
                (pred_head_df['validation'] != 999) &
                (pred_head_df['label'] == "detection-miss") &
                (
        ...
        valid_df = copy.deepcopy(pred_head_df[
            (pred_head_df['validation'] == num_cv) &
            (
                (pred_head_df['unique_key'].str.contains('fixed-point-camera')) |
                (pred_head_df['unique_key'].str.contains('for-learning/2023-11-19-omaezaki-500')) |
...
if __name__ == '__main__':
    ...
    train_cv(annotation_head_df, pred_head_df)
